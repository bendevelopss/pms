...
$content2=mysql_query("SELECT * FROM pullout where pullout_no= ".$_GET['id']." ");
...
$this->Cell(100, 8, 'E-mail Address: persan_inc@yahoo.com', 0,'C');
  $this->Text(130,37,'Pullout No.',0,0,'l');
  $this->SetFont('Arial','u',11);
  $this->Text(170,37,''.$_GET['pullout_no'].'',0,0,'l');
...
$this->Text(35,68,''.$row['customer'].'',0,0,'C');
  $this->SetFont('Arial','',11);
    ...
    $this->SetFont('Arial','u',11);
$this->Text(30,74,''.$row['project'].'',0,0,'C');
  $this->SetFont('Arial','',11);
...
$productByCode = $db_handle->runQuery("SELECT * , SUM(quantity) as total FROM pullout_cart where pullout_no= ".$_GET['pullout_no']." group by material_no");
