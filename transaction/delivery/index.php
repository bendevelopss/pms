              ...
              <div class="col-lg-12 col-sm-12 col-xs-12">
                <div class="box box-solid">
                  <div class="box-header">
                    <h3 class="box-title">Browse Delivery</h3>
...
<?php
                    ...
                    while($row = $result->fetch_assoc())
                      { 
                       ...
                       echo '<tr>'; 
                        echo'<td>' .str_pad($row['po'], 4, '0', STR_PAD_LEFT).'</td>';
        echo'<td>'.ucfirst($row['supp']).' </td>';
        echo'<td>'.ucfirst($row['date']).'</td>';
        echo'<td>'.ucfirst($row['ordered']).'</td>';
                       echo'<td>';
                          echo'<button type="button" name="btnNext" onclick="return myFunctions();" class="btn btn-primary"><a href="../delivery_accept/index.php?id='.$hell.'&scname='.$row['po'].'&prepared='.ucfirst($lastname2).', '.ucfirst($firstname2).' '.strtoupper($middlename2[0]).'" style="color:White; ">Process Delivery</a></button></td> ';
       }
